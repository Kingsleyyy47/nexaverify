...
// Plain GET-style search — submitting pushes `?q=...` onto the URL, which
// app/admin/transactions/page.js reads server-side and re-queries with.
// Kept as a separate client component so the page itself can stay a Server
// Component (needs no client-side state of its own beyond the input).
export default function AdminTransactionsFilterBar({ initialQuery = "" }) {
  ...
  function handleSubmit(e) {
    ...
    router.push(trimmed ? `/admin/transactions?q=${encodeURIComponent(trimmed)}` : "/admin/transactions");
  }
