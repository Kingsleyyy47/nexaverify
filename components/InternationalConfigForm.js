...
// `config` comes from admin/international/page.js as:
// { enabled, markupAmountNgn, updatedAt }
export default function InternationalConfigForm({ config }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("/api/admin/international/config", {
        method: "POST",
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-lg">
      <div className="pb-5 border-b border-gray-100 dark:border-night-800">
        <div className="flex items-center justify-between mb-2">
          <span className="font-bold text-sm">International numbers</span>
        ...
        <p className="text-xs text-gray-400 dark:text-night-400">
          Turns the "International Numbers" product on or off for customers. Backed by DaisySim —
          country+service scoped virtual numbers with their own live pricing, separate from the
          regular DaisySMS catalog.
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 dark:text-night-400 mt-1.5">
          DaisySim returns prices in USD. This flat Naira amount is added on top of the
          USD-to-NGN converted price before it's shown to customers or charged to their wallet.
        </p>
      </div>
