...
export function parseAndValidateAccountsCsv(csvText) {
  ...
  // Drop bare "label" lines — a single field with none of the chosen
  ...
  const meaningful = indexed.filter((r) => r.raw.length > 1);
  const rows = meaningful.length > 0 ? meaningful : indexed;
...
function parseHeaderlessRows(rows) {
  ...
  rows.forEach(({ raw, rowNumber }) => {
    ...
    template.forEach((field, i) => {
      ...
      values[field] = (trimmedRaw[i] || "").trim();
    });
