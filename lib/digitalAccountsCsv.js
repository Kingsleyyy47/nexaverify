...
//
// Three input shapes are supported, auto-detected — no format picker needed:
//
...
//    TXT.
//
// 3. SINGLE-COLUMN identifier list: no header, no delimiter at all — every
//    line is just ONE value, e.g. a plain list of emails, usernames, or
//    login links with nothing else alongside them (some products are sold
//    as bare handles/links/email lists, not full username+password
//    credentials). Each line is auto-detected as a login link
//    (looksLikeLink), an email (looksLikeEmail), or — falling back —
//    a username, per the business owner's request ("it should detect
//    whichever"). No password is stored for this shape at all
//    (digital_stock_items.password is nullable specifically for this — see
//    schema.sql) rather than guessing or requiring one that was never
//    provided. See parseSingleColumnRows below.
//
// If none of the five delimiters ever splits a line at all AND the file
// doesn't parse as shape 3 either (i.e. every line really is empty or
// unusable), that's reported as its own specific error rather than silently
// defaulting to comma and misreporting it as an unrecognized "1 column"
// format (the actual bug behind the Sept 2026 "still hardcodes placeholders"
// report — a file using a delimiter outside the original comma/pipe/colon
// set, or genuinely undelimited text, silently fell back to comma, which
// doesn't appear anywhere in the file, so every line collapsed to one
// column). In practice shape 3 now catches every genuinely undelimited file
// that has at least one usable line, so that old error only fires for a
// truly empty/unusable file.
...
// "Mail password", "Recovery Mail", "2fa key", "No of friends"). "Login" (and
// its close variants) is a very common stand-in for "Username" specifically
// as the account identifier — some product files use one, some the other,
// sometimes alongside an "Email" column instead — so it maps straight to
// "username", making it just another way to satisfy the same
// email-or-username requirement (see REQUIRED_EITHER/validateAccountFields)
// rather than a separate required field of its own. Deliberately distinct
// from "login_link"/"login_url"/etc. below, which are a real clickable URL,
// not an identifier.
...
export function parseAndValidateAccountsCsv(csvText) {
  ...
  // Checked BEFORE normal delimiter detection, not just as a fallback when
  // it fails — a bare link like "https://facebook.com/profile1" contains a
  // colon (from "https:"), and if colon also happens to be a viable
  // delimiter elsewhere in the file, the normal detector would confidently
  // (and wrongly) split every link in two at that colon instead of ever
  // reaching the single-column path below. isSingleColumnFile only
  // qualifies a file when EVERY line is safely one whole value — a real
  // link, a real whole email, or a line with none of the five delimiter
  // characters in it at all — so a genuine multi-column file (where lines
  // contain a delimiter but aren't links/emails) is never short-circuited
  // here.
  if (isSingleColumnFile(text)) {
    return parseSingleColumnRows(text);
  }
...
// Whole-line check used to decide, up front, whether the ENTIRE file is
// shape 3 (single-column) — see parseAndValidateAccountsCsv above for why
// this has to run before, not just after, normal delimiter detection. A
// line qualifies as "safely one value" if it's a real link (looksLikeLink —
// already anchored to the whole trimmed value, and already excludes
// whitespace/";" internally) or contains none of the five delimiter
// characters at all. A real email never contains any of those five
// characters either, so it's already covered by the second case — no
// separate email pattern needed here (and deliberately so: matching email
// loosely against the whole line would wrongly accept "user@mail.com,pass"
// as one bare email, since a plain @/./greedy pattern doesn't know comma
// isn't part of it — the per-line classification in parseSingleColumnRows
// below is what actually decides link vs email vs username, only after
// this check has confirmed every line is a single value in the first
// place). The file only qualifies when EVERY non-blank line passes — a
// single line that has a delimiter character but isn't a link means the
// file has real columns, so normal delimiter detection has to run instead.
const ANY_DELIMITER_CHAR_RE = /[,|:;\t]/;

function isSingleColumnFile(text) {
  const lines = text
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l !== "");
  if (lines.length === 0) return false;
  return lines.every((line) => looksLikeLink(line) || !ANY_DELIMITER_CHAR_RE.test(line));
}

// Shape 3 — see the file header comment. Reached when isSingleColumnFile
// (above) confirms every line is just one bare value — either up front, or
// as a fallback when normal delimiter detection found NONE of
// comma/pipe/colon/semicolon/tab acting as a real separator anywhere. Each
// line is classified independently (a file can freely mix links, emails,
// and plain usernames line to line — there's no shared column layout to
// agree on since there's only ever one column):
//   - a real login link (looksLikeLink)              -> login_link
//   - an email address (looksLikeEmail)               -> email
//   - anything else, EXCEPT cookie/session-looking junk -> username
// No password column exists in this shape at all, so password is always
// null (digital_stock_items.password is nullable specifically for this —
// see schema.sql) rather than guessed or force-required. A line that looks
// like cookie/session data on its own (no link, no email, just a stray
// session blob) is rejected with its own row error instead of being swept
// in as a bogus "username" — same "reject clearly, never guess" rule as
// every other path in this file.
function parseSingleColumnRows(text) {
  const lines = text
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l !== "");
  ...
  lines.forEach((value, i) => {
    ...
    if (looksLikeCookieOrSessionData(value)) {
      errors.push({
        row: rowNumber,
        message: "This looks like cookie/session data, not a link, email, or username.",
      });
      return;
    }
    items.push({
  ...
  if (errors.length > 0) {
    return { items: [], errors, categoryHint: null };
  }
  if (items.length === 0) {
    return { items: [], errors: [{ row: 0, message: "No account rows found in the file." }], categoryHint: null };
  }
  return { items, errors: [], categoryHint: null };
}
