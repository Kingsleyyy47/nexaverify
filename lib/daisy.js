...
// /api/info) that live on the same domain but outside handler_api.php.
...
//
// Docs: https://daisysms.io/docs/api (the .com mirror at daisysms.com/docs/api
// has the full "Long-term rentals / LTR" section, which is where duration,
// renewable, auto_renew, keep, setAutoRenew and getLtrs come from).
//
...
const API_ORIGIN = new URL(BASE_URL).origin; // e.g. https://daisysms.io
...
export async function getLtrs() {
  ...
  const url = new URL(`${API_ORIGIN}/api/ltrs`);
  url.searchParams.set("api_key", API_KEY);
  const res = await fetchWithTimeout(url.toString(), { method: "GET", cache: "no-store" });
...
export async function getAccountInfo() {
  if (!API_KEY) throw new DaisyError("NO_API_KEY", "DAISYSMS_API_KEY is not set in the environment");
  const url = new URL(`${API_ORIGIN}/api/info`);
  url.searchParams.set("api_key", API_KEY);
  const res = await fetchWithTimeout(url.toString(), { method: "GET", cache: "no-store" });
  const text = await res.text();
  return safeJson(text);
