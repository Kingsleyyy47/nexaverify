...
export async function creditVirtualAccountFromWebhook({ accountNumber, reference, amountNgn }) {
  ...
  // Sept 2026 incident, part 1: this SELECT's `error` used to be silently
  // dropped (`const { data: accounts } = await ...`), so a transient blip
  // talking to Postgres made a real deposit look identical to "no such
  // account" — `accounts` fell back to `[]`, `match` came back undefined, and
  // the customer's money vanished with zero trace in any table or log. One
  // retry covers the transient case; if it fails twice in a row, log it
  // loudly instead of guessing.
  //
  // Sept 2026 incident, part 2: this used to fetch EVERY row in
  // virtual_accounts (`.select(...).eq("provider", "pocketfi")` with no
  // filter on account_number) and find the match in JavaScript. That worked
  // fine while the table was small, but Supabase's REST layer caps an
  // unfiltered select at 1000 rows by default — once virtual_accounts
  // crossed that count (around Sept 24), the query silently came back with
  // only a partial, arbitrarily-ordered slice of the table, so a real,
  // correctly-provisioned account could simply not be in the batch that
  // happened to come back, with no error at all. Filtering by account_number
  // directly in the query means Postgres does the matching, not us, so table
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    const result = await admin
      ...
      .eq("provider", "pocketfi")
      .or(`account_number.eq.${accountNumber},account_number.eq.${digitsOnly}`)
      .limit(5);
