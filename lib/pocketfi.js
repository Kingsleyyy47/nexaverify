...
//
// This file is server-only: it reads POCKETFI_PUBLIC_KEY from the
// environment and must never be imported from a Client Component.
//
// CORRECTED (was backwards before): PocketFi's naming is the reverse of what
// the labels suggest. The "Public API Key" (Laravel Sanctum-style `id|token`
// credential, e.g. "1|abc123...") is what actually authenticates outbound API
// calls as a Bearer token — that's POCKETFI_PUBLIC_KEY below. The "Secret API
// Key" (a plain hex string) is NOT a valid bearer token at all; it's only
// used to verify the HMAC-SHA512 signature on INBOUND webhook payloads — see
// POCKETFI_SECRET_KEY in app/api/pocketfi/webhook/route.js, not here. An
// earlier version of this integration had these swapped (used the Secret Key
// as the Bearer token here), which is what caused intermittent
// "Unauthenticated." rejections — don't swap them back.
//
...
const PUBLIC_KEY = process.env.POCKETFI_PUBLIC_KEY;
...
async function call(path, body) {
  if (!PUBLIC_KEY) {
    throw new PocketfiError("NO_API_KEY", "POCKETFI_PUBLIC_KEY is not set in the environment");
  }
  ...
  try {
    ...
    // Never bake the raw response body into the thrown error's message — a
    // non-JSON response is exactly what a Cloudflare/WAF challenge page or
    // other HTML block looks like, and PocketfiError.message can reach a
    // customer via lib/wallet-funding.js's callers. Log the real text
    // server-side only.
    console.error(`[pocketfi] non-JSON response:`, text.slice(0, 500));
    throw new PocketfiError("BAD_JSON", "PocketFi sent back an unexpected response.");
  }
...
// Creates a dedicated (permanent) NUBAN bank account for one customer to
// fund their wallet by bank transfer, any time, without a checkout redirect.
// NOTE: PocketFi's docs use camelCase "businessId" for this endpoint (and
// for /virtual-accounts/fetch), but snake_case "business_id" for
// /checkout/request above — that's not a typo on our end, it's what their
// own docs show for each endpoint respectively. Don't "fix" this to be
// consistent; send whichever casing the docs show for whichever endpoint.
//
// bank must be one of: saveheaven, paga, kuda, 9psb, palmpay. nin/bvn are
// optional except REQUIRED when bank is "palmpay" — omit both for the other
// four providers.
export async function createVirtualAccount({
  ...
  const data = await call("/virtual-accounts/create", {
    first_name: firstName,
    last_name: lastName,
    phone,
    email,
  ...
  if (!bankInfo?.accountNumber) {
    throw new PocketfiError("UNKNOWN_RESPONSE", "PocketFi response is missing an account number");
  }
...
// GET /virtual-accounts/fetch — lists every virtual account already created
// for this business. Not used by the customer-facing get-or-create flow
// (we track the one-per-user mapping ourselves in public.virtual_accounts),
// but useful for admin-side reconciliation if that mapping ever needs
// double-checking against PocketFi's own records.
export async function fetchVirtualAccounts() {
  ...
  const res = await fetchWithTimeout(
    `${BASE_URL}/virtual-accounts/fetch?businessId=${encodeURIComponent(BUSINESS_ID)}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${PUBLIC_KEY}`,
  ...
  try {
    ...
    console.error(`[pocketfi] non-JSON response (fetchVirtualAccounts):`, text.slice(0, 500));
    throw new PocketfiError("BAD_JSON", "PocketFi sent back an unexpected response.");
  }

  if (!res.ok || data?.status === false) {
    throw new PocketfiError("REQUEST_FAILED", data?.message || `PocketFi returned HTTP ${res.status}`);
  }
