...
async function call(method, path, body) {
  ...
  try {
    ...
    // other HTML block looks like, and this error's .message reaches a
    // customer on /products/international if left as-is. Log the real text
    // server-side only.
    console.error(`[daisysim] non-JSON response:`, text.slice(0, 500));
    throw new DaisySimError("BAD_JSON", "DaisySim sent back an unexpected response.");
  }
