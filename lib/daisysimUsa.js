...
async function call(method, path, body) {
  ...
  try {
    ...
    // other HTML block looks like, and this is the exact provider that
    // leaked a raw "returned HTTP 4xx" fallback to a customer in Sept 2026.
    // Log the real text server-side only; the customer-facing message stays
    // generic no matter what the API actually sent back.
    console.error(`[daisysimUsa] non-JSON response:`, text.slice(0, 500));
    throw new DaisySimUsaError("BAD_JSON", "DaisySim USA sent back an unexpected response.");
  }
...
export async function getApps(country = "USA") {
  ...
  // Defensive dedup by code — every consumer treats `code` as a unique
  // identifier (see the identical comment in lib/getatext.js, where this
  // provider's sibling was observed returning the same code more than once
  // in one response and broke the admin catalog sync's upsert as a result).
  // Not documented as a real possibility for this API, but cheap insurance
  // against the exact same class of failure.
  const seen = new Set();
  return rawApps.filter((a) => {
