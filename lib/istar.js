...
// isn't even a per-quantity-tier endpoint (see buildPremiumPricing above).
// The only way to ever learn the real cost is to look at what a completed
// order actually charged, which is why istar_config carries a
// self-learning `star_last_cost_ngn` alongside the static `ngn_per_star`
// fallback. This just decides which of the two to use — the actual writing
// of star_last_cost_ngn happens in learnStarCostFromOrder below.
//
// `config` is { ngnPerStar, starMarkupNgn, starLastCostNgn }. Returns the
// price for a SINGLE star, in NGN — multiply by quantity for the total.
//
// Always ADDS the markup on top of whichever base cost is available — never
// multiplies, and never drops the markup silently. Base cost is the learned
// real cost once one exists, otherwise the admin's starting-price guess.
export function computeStarPricePerUnit(config) {
  ...
  const markup = Number(config.starMarkupNgn || 0);
...
// Only learns from USDT orders: USDT is pegged ~1:1 to USD, so
// (amount / quantity) x currency_rates.USD is a reliable NGN conversion.
// TON orders are skipped entirely — there's no TON->NGN rate anywhere in
// this app, and guessing one would be worse than not learning at all.
...
export async function learnStarCostFromOrder(admin, { quantity, amount, walletType }) {
  if (walletType !== "USDT" || !quantity || !amount || Number(quantity) <= 0) return false;
  ...
  if (!usdRate) return false;

  const perStarNgn = (Number(amount) / Number(quantity)) * usdRate;
  if (!Number.isFinite(perStarNgn) || perStarNgn <= 0) return false;
  ...
  await admin
    ...
    .update({
      ...
      star_last_cost_updated_at: new Date().toISOString(),
    })
    .eq("id", true);
