...
export function computeStarPricePerUnit(config) {
  if (config.starLastCostNgn != null && Number(config.starLastCostNgn) > 0) {
    return Math.round((Number(config.starLastCostNgn) + Number(config.starMarkupNgn || 0)) * 10000) / 10000;
  }
  return Number(config.ngnPerStar || 0);
}
