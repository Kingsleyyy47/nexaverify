...
async function call(method, path, body) {
  ...
  try {
    ...
    // other HTML block looks like (the Sept 2026 incident that started this
    // whole safeguard), and several call sites eventually show err.message
    // to a customer. Log the real text server-side only; the customer-facing
    // message stays generic no matter what Getatext actually sent back.
    console.error(`[getatext] non-JSON response:`, text.slice(0, 500));
    throw new GetatextError("BAD_JSON", "Getatext sent back an unexpected response.");
  }
...
export async function getApps() {
  ...
  // Getatext's own catalog has been observed returning the same api_name
  // more than once (Sept 2026 — thousands of entries, many duplicate codes,
  // possibly one row per price tier/lot under the same service). Every
  // consumer treats `code` as a unique identifier — the admin catalog sync
  // (app/api/admin/us-only/sync) upserts one row per code and fails outright
  // with Postgres's "ON CONFLICT DO UPDATE command cannot affect row a
  // second time" if the same code appears twice in one batch, and the
  // customer-facing catalog would otherwise show the same service listed
  // more than once. Keep the FIRST occurrence of each code — Getatext's
  // ordering isn't documented, but first-seen is at least deterministic
  // across calls with the same response.
  const seen = new Set();
...
// own backend timeout sweep runs at 15 minutes — see
// lib/rentalTimeout.js#RENTAL_BACKEND_TIMEOUT_MINUTES — so on an account
// without instant cancellation, a manual cancel attempted in that first
// 5-minute window can still fail, but by the time the 15-minute sweep runs
// that window has always already closed).
